...
#include "TrafficCity.h"
#include <TrafficCity/ai/evolution_manager.h>

//#include "RaycastVehicle.h"
#include "TrafficCity/network/Vehicle.h"
...
#include <TrafficCity/beat/AudioSpectrumAnalyzer.h>
#include <TrafficCity/beat/AudioSpectrumAxis.h>
#include <TrafficCity/beat/AudioSpectrumRender.h>
#include <TrafficCity/beat/AudioSpectrumCapture.h>
...
#include "Missile.h"
#include "TrafficCity/beat/BufferData.h"
#include <TrafficCity/PixelImage.h>
#include <TrafficCity/Constants.h>
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
URHO3D_DEFINE_APPLICATION_MAIN(TrafficCity)

TrafficCity::TrafficCity(Context *context) :
        Game(context),
...
void TrafficCity::Setup() {
    Game::Setup();
}

// SETUP AI BOTS
void TrafficCity::InitEvolutionManager() {
...
void TrafficCity::ShowEvolutionManagerStats() {
...
void TrafficCity::CreateEmptyScene(Context* context) {
...
void TrafficCity::UpdateUIState(bool state) {
...
Node* TrafficCity::GenerateObject(Vector3 &pos, Quaternion &rot, int type)
{
...
void TrafficCity::PlaceObject(Camera* pCamera, int type)
{
    Vector3 hitPos;
    ...
    if (Raycast(pCamera, 2500000.0f, hitPos, hitDrawable))
    {
        ...
        URHO3D_LOGDEBUGF("TrafficCity::PlaceObject()->GenerateObject() @ [%f, %f, %f]", hitPos.x_, hitPos.y_, hitPos.z_);
    }
}

bool TrafficCity::Raycast(Camera* pCamera, float maxDistance, Vector3& hitPos, Drawable*& hitDrawable)
{
    hitDrawable = nullptr;
...
void TrafficCity::HandleResourceBackgroundLoaded(StringHash eventType, VariantMap& eventData) {
...
void TrafficCity::HandleClientResourceLoadAttempt(StringHash eventType, VariantMap& eventData) {
...
void TrafficCity::HandleClientResourceLoadFinished(StringHash eventType, VariantMap& eventData) {
...
void TrafficCity::HandleClientSceneLoaded(StringHash eventType, VariantMap& eventData)
{
    // SERVER CODE
...
void TrafficCity::SubscribeToEvents() {

    // Subscribe to UI element events
    //SubscribeToEvent(textEdit_, E_TEXTFINISHED, URHO3D_HANDLER(TrafficCity, HandleSendChat));
//    SubscribeToEvent(sendButton_, E_RELEASED, URHO3D_HANDLER(TrafficCity, HandleSend));

    // Last chance to update before back buffer flip
//    SubscribeToEvent(E_ENDRENDERING, URHO3D_HANDLER(TrafficCity, HandleEndRendering));
    SubscribeToEvent(E_BEGINRENDERING, URHO3D_HANDLER(TrafficCity, HandleEndRendering));
    //E_BEGINRENDERING

    // Server asset loading
    SubscribeToEvent(E_RESOURCEBACKGROUNDLOADED, URHO3D_HANDLER(TrafficCity, HandleResourceBackgroundLoaded));

    // Client asset loading
    SubscribeToEvent(E_LOADSUCCEED, URHO3D_HANDLER(TrafficCity, HandleClientResourceLoadFinished));
    SubscribeToEvent(E_LOADATTEMPT, URHO3D_HANDLER(TrafficCity, HandleClientResourceLoadAttempt));

    // Subscribe to log messages so that we can pipe them to the chat window
    SubscribeToEvent(E_LOGMESSAGE, URHO3D_HANDLER(TrafficCity, HandleLogMessage));

    // Subscribe to network events
    SubscribeToEvent(E_NETWORKMESSAGE, URHO3D_HANDLER(TrafficCity, HandleNetworkMessage));
    SubscribeToEvent(E_CLIENTSCENELOADED, URHO3D_HANDLER(TrafficCity, HandleClientSceneLoaded));
    SubscribeToEvent(E_CLIENTCONNECTED, URHO3D_HANDLER(TrafficCity, HandleClientConnected));

    // Subscribe to button actions
    SubscribeToEvent(playButton_, E_RELEASED, URHO3D_HANDLER(TrafficCity, HandlePlayButton));
//    SubscribeToEvent(connectButton_, E_RELEASED, URHO3D_HANDLER(TrafficCity, HandleConnect));
    SubscribeToEvent(disconnectButton_, E_RELEASED, URHO3D_HANDLER(TrafficCity, HandleDisconnect));
    SubscribeToEvent(startServerButton_, E_RELEASED, URHO3D_HANDLER(TrafficCity, HandleStartServer));
    SubscribeToEvent(exitButton_, E_RELEASED, URHO3D_HANDLER(TrafficCity, HandleExit));

    SubscribeToEvent(E_CONNECTFAILED, URHO3D_HANDLER(TrafficCity, HandleConnectionFailed));

    // Subscribe to server events
    SubscribeToEvent(E_SERVERSTATUS, URHO3D_HANDLER(TrafficCity, HandleConnectionStatus));

    // Subscribe to player state events
    SubscribeToEvent(E_PLAYERSTATE, URHO3D_HANDLER(TrafficCity, HandlePlayerStateUpdate));
    ...
    SubscribeToEvent(E_PLAYERPICKUPSTATE, URHO3D_HANDLER(TrafficCity, HandlePlayerPickUpState));
    // Events sent between client & server (remote events) must be explicitly registered or else they are not allowed to be received
    GetSubsystem<Network>()->RegisterRemoteEvent(E_PLAYERPICKUPSTATE);

    // E_CHATMESSAGE
    // Subscribe to chat message events
    SubscribeToEvent(E_CHATMESSAGE, URHO3D_HANDLER(TrafficCity, HandleChatMessage));
    // Events sent between client & server (remote events) must be explicitly registered or else they are not allowed to be received
    GetSubsystem<Network>()->RegisterRemoteEvent(E_CHATMESSAGE);


    SubscribeToEvent(E_CLIENTOBJECTID, URHO3D_HANDLER(TrafficCity, HandleClientObjectID));
    // Events sent between client & server (remote events) must be explicitly registered or else they are not allowed to be received
    GetSubsystem<Network>()->RegisterRemoteEvent(E_CLIENTOBJECTID);

    SubscribeToEvent(E_LOGINLISTREFRESH, URHO3D_HANDLER(TrafficCity, HandleLoginListRefresh));
    // Events sent between client & server (remote events) must be explicitly registered or else they are not allowed to be received
    GetSubsystem<Network>()->RegisterRemoteEvent(E_LOGINLISTREFRESH);


    SubscribeToEvent(E_NETWORKUPDATESENT, URHO3D_HANDLER(TrafficCity, HandleNetworkUpdateSent));

    // Subscribe HandlePostUpdate() function for processing post update events
    SubscribeToEvent(E_POSTUPDATE, URHO3D_HANDLER(TrafficCity, HandlePostUpdate));

    // Subscribe to PostRenderUpdate to draw debug geometry
    SubscribeToEvent(E_POSTRENDERUPDATE, URHO3D_HANDLER(TrafficCity, HandlePostRenderUpdate));

  //  SubscribeToEvent(E_JOYSTICKHATMOVE, URHO3D_HANDLER(TrafficCity, HandleJoystickHatMove));


    // Subscribe to fixed timestep physics updates for setting or applying controls
    SubscribeToEvent(E_PHYSICSPRESTEP, URHO3D_HANDLER(TrafficCity, HandlePhysicsPreStep));

    // Subscribe the update event for the delayed events
    //SubscribeToEvent(E_POSTUPDATE, URHO3D_HANDLER(TrafficCity, HandleDelayUpdate));

}

// CLIENT CODE
// Sample client input
Controls TrafficCity::SampleCSPControls()
{
    auto ui = GetSubsystem<UI>();
...
void TrafficCity::HandleAI(float timeStep) {
    auto *input = GetSubsystem<Input>();
...
void TrafficCity::HandlePhysicsPreStep(StringHash eventType, VariantMap &eventData) {
...
// CLIENT CODE
void TrafficCity::HandleLoginListRefresh(StringHash eventType, VariantMap &eventData) {
...
// CLIENT CODE
void TrafficCity::HandleRaceRank(StringHash eventType, VariantMap &eventData) {
...
// CLIENT CODE
void TrafficCity::HandleRaceVictory(StringHash eventType, VariantMap &eventData) {
...
// CLIENT CODE
void TrafficCity::HandleRadioTrackNext(StringHash eventType, VariantMap &eventData) {
...
void TrafficCity::HandleNetworkUpdateSent(StringHash eventType, VariantMap& eventData)
{
...
void TrafficCity::HandleNodeCollision(StringHash eventType, VariantMap& eventData) {
...
void TrafficCity::HandlePlayerStateUpdate(StringHash eventType, VariantMap& eventData) {
...
void TrafficCity::HandleClientUIUpdate() {
...
void TrafficCity::HandlePlayerPickUpState(StringHash eventType, VariantMap& eventData) {
...
void TrafficCity::HandleClientConnected(StringHash eventType, VariantMap& eventData)
{
...
void TrafficCity::HandleCollisionBegin(StringHash eventType, VariantMap &eventData) {
    // Get colliding node
...
void TrafficCity::SetParticleEmitter(int hitId, float contactX, float contactY, int type, float timeStep) {
    // CREATE
    auto *cache = GetSubsystem<ResourceCache>();
...
void TrafficCity::HandleUpdateParticlePool(float timeStep) {
    // CREATE
    auto *cache = GetSubsystem<ResourceCache>();
...
void TrafficCity::HandleCollisionEnd(StringHash eventType, VariantMap &eventData) {
    // Get colliding node
...
void TrafficCity::HandleRenderUpdate(StringHash eventType, VariantMap &eventData) {
    using namespace Update;
    auto *input = GetSubsystem<Input>();
...
void TrafficCity::DrawWave(const Vector2 &start, int length, int pixelAmplitude, float t, const Color &color) {
...
void TrafficCity::DrawFFT(const Vector2 &start, int length, int pixelAmplitude, const Color &color) {
    Vector<Vector2> points;
...
void TrafficCity::HandleDelayUpdate(StringHash eventType, VariantMap& eventData) {
    using namespace Update;
    auto *input = GetSubsystem<Input>();
...
void TrafficCity::HandleUpdate(StringHash eventType, VariantMap &eventData) {
    using namespace Update;
    auto *input = GetSubsystem<Input>();
...
//        drawDebug_ = !drawDebug_;
    ...
    if (input->GetKeyPress(KEY_K)) {
        File saveFile(context_, GetSubsystem<FileSystem>()->GetProgramDir() + "Data/Scenes/TrafficCity_Output.xml",
                      FILE_WRITE);
...
void TrafficCity::HandlePostUpdate(StringHash eventType, VariantMap &eventData) {
...
void TrafficCity::HandlePostRenderUpdate(StringHash eventType, VariantMap &eventData) {
...
void TrafficCity::HandleEndRendering(StringHash eventType, VariantMap &eventData) {
...
void TrafficCity::ReloadScene(bool reInit) {
  // LOAD MAP
...
// LOAD BASE
 ...
 //////////
        // sub
        SubscribeToEvent(E_ASYNCLOADPROGRESS, URHO3D_HANDLER(TrafficCity, HandleLoadProgress));
        SubscribeToEvent(E_ASYNCLEVELOADFINISHED, URHO3D_HANDLER(TrafficCity, HandleLevelLoaded));
   ...
   void TrafficCity::LoadLevel(int id) {
       auto *cache = GetSubsystem<ResourceCache>();
       levelPathName_= "Scenes/Async/";
...
//    String levelPathFile = levelPathName_ + loadLevel + String(".xml");
   ...
   void TrafficCity::PlaySoundEffect(const String &soundName) {
...
void TrafficCity::PlayDrumMachine() {
    // CLIENT CODE
...
void TrafficCity::PlayMusic(const String &soundName) {
    // CLIENT CODE
...
// SERVER CODE
void TrafficCity::PlaySoundEffectGlobal(const String &soundName) {
...
// CLIENT CODE
SoundSource3D *TrafficCity::PlaySoundEffectLocal(const String &soundName) {
...
void TrafficCity::HandlePlayButton(StringHash eventType, VariantMap &eventData) {
...
// Network functions
void TrafficCity::CreateServerSubsystem() {
...
void TrafficCity::InitiateViewport(Context* context, Scene* scene, Camera* camera, int id)
{
...
void TrafficCity::SetupSequencerViewport() {
    auto *graphics = GetSubsystem<Graphics>();
    auto *renderer = GetSubsystem<Renderer>();

    // Load menu scene
    ResourceCache *cache = GetSubsystem<ResourceCache>();
    XMLFile *xmlLevel = cache->GetResource<XMLFile>("Scenes/TrafficCitySequencer.xml");
...
void TrafficCity::SetupMenuViewport() {
    auto* graphics = GetSubsystem<Graphics>();
    auto* renderer = GetSubsystem<Renderer>();

    // Load menu scene
    ResourceCache *cache = GetSubsystem<ResourceCache>();
    XMLFile *xmlLevel = cache->GetResource<XMLFile>("Scenes/TrafficCityMenu.xml");
...
void TrafficCity::SetupGameViewports()
{
...
void TrafficCity::ChangeDebugHudText() {
...
Button *TrafficCity::CreateButton(const String &text, int width) {
    ResourceCache *cache = GetSubsystem<ResourceCache>();
...
void TrafficCity::UpdateButtons() {
    Network *network = GetSubsystem<Network>();
    Connection *serverConnection = network->GetServerConnection();
...
void TrafficCity::MoveCamera(float timeStep) {
...
void TrafficCity::NodeRegisterLoadTriggers(Node *node)
{
    if (node)
    {
        ...
        {
            SubscribeToEvent(result[i], E_NODECOLLISIONSTART, URHO3D_HANDLER(TrafficCity, HandleLoadTriggerEntered));
        }
    }
    else
    {
...
void TrafficCity::HandleLoadTriggerEntered(StringHash eventType, VariantMap& eventData)
{
...
void TrafficCity::HandleLoadProgress(StringHash eventType, VariantMap& eventData)
{
...
void TrafficCity::HandleLevelLoaded(StringHash eventType, VariantMap& eventData) {
    ResourceCache *cache = GetSubsystem<ResourceCache>();
...
void TrafficCity::HandleConnectionFailed(StringHash eventType, VariantMap &eventData) {
    URHO3D_LOGINFO("Connection to server failed!");
    InitMsgWindow("Connection failure", "Connection to server failed!");
}

void TrafficCity::DoConnect() {
...
//    String address = textEdit_->GetText().Trimmed();
    ...
    if (network->Connect(address, SERVER_PORT, scene_, identity)) {
        ...
        URHO3D_LOGINFOF("TrafficCity::HandleConnect - data: [%s, %d]", name.CString(), idx);
...
//        textEdit_->SetText(String::EMPTY);
    ...
    SubscribeToEvent(E_SERVERCONNECTED, URHO3D_HANDLER(TrafficCity, HandleServerConnected));
}

void TrafficCity::HandleConnect(StringHash eventType, VariantMap &eventData) {
   DoConnect();
}

void TrafficCity::HandleDisconnect(StringHash eventType, VariantMap &eventData) {
    Server *server = GetSubsystem<Server>();
    server->Disconnect();
...
void TrafficCity::DoStartServer() {
    Server *server = GetSubsystem<Server>();
...
void TrafficCity::HandleStartServer(StringHash eventType, VariantMap &eventData) {
    // SERVER CODE STARTS
    DoStartServer();
}

void TrafficCity::HandleConnectionStatus(StringHash eventType, VariantMap &eventData) {
...
void TrafficCity::HandleClientObjectID(StringHash eventType, VariantMap &eventData) {
    // CLIENT CODE
    ...
    if (eventType == E_CLIENTOBJECTID) {
        ...
        URHO3D_LOGINFOF(
                "Client -> TrafficCity::HandleClientObjectID: networkActorNodeId_ = %l, vehicleNodeId_ = %l",
                networkActorNodeId_, vehicleNodeId_);

    }
}

void TrafficCity::HandleExit(StringHash eventType, VariantMap &eventData) {
    engine_->Exit();
}

void TrafficCity::StartMultiplayerGameSession()
{
    // Multiple session
    isMultiplayer_ = true;
    ...
    if (network->IsServerRunning())
    {
        if (!handlersSet) {
            ...
            SubscribeToEvent(E_CLIENTIDENTITY, URHO3D_HANDLER(TrafficCity, HandleClientIdentity));
            // Subscribe the update event for the whole logic
            SubscribeToEvent(E_UPDATE, URHO3D_HANDLER(TrafficCity, HandleUpdate));
            // Subscribe for the client disconnection
            SubscribeToEvent(E_CLIENTDISCONNECTED, URHO3D_HANDLER(TrafficCity, HandleClientDisconnected));
    ...
    else if (network->GetServerConnection())
    {
        ...
        SubscribeToEvent(E_PLAYERSPAWNED, URHO3D_HANDLER(TrafficCity, HandlePlayerRespawned));
        ...
        SubscribeToEvent(E_PLAYERLOADED, URHO3D_HANDLER(TrafficCity, HandlePlayerLoaded));

        // Register the remote event
        network->RegisterRemoteEvent(E_RACERANK);
        // Subscribe for the remote event 'race rank'
        SubscribeToEvent(E_RACERANK, URHO3D_HANDLER(TrafficCity, HandleRaceRank));

        // Register the remote event
        network->RegisterRemoteEvent(E_RACEVICTORY);
        // Subscribe for the remote event 'race rank'
        SubscribeToEvent(E_RACEVICTORY, URHO3D_HANDLER(TrafficCity, HandleRaceVictory));

        // Register the remote event
        network->RegisterRemoteEvent(E_RADIOTRACKNEXT);
        // Subscribe for the remote event 'player loaded'
        SubscribeToEvent(E_RADIOTRACKNEXT, URHO3D_HANDLER(TrafficCity, HandleRadioTrackNext));
...
void TrafficCity::InitMsgWindow(String title, String message) {
    UI *ui = GetSubsystem<UI>();
    ResourceCache *cache = GetSubsystem<ResourceCache>();
...
//    windowTitle->SetFont(cache->GetResource<Font>("Fonts/SinsGold.ttf"), 15);
    ...
    SubscribeToEvent(buttonClose, E_RELEASED, URHO3D_HANDLER(TrafficCity, HandleClosePressed));

    // Subscribe also to all UI mouse clicks just to see where we have clicked
    //  SubscribeToEvent(E_UIMOUSECLICK, URHO3D_HANDLER(HelloGUI, HandleControlClicked));
}

void TrafficCity::HandleClosePressed(StringHash eventType, VariantMap &eventData) {
    // Shutdown
    engine_->Exit();
}


void TrafficCity::OutputLoginListToConsole() {
...
void TrafficCity::ShowChatText(const String &row) {
...
void TrafficCity::HandleLogMessage(StringHash /*eventType*/, VariantMap &eventData) {
...
void TrafficCity::HandleSendChat() {
...
void TrafficCity::HandleNetworkMessage(StringHash /*eventType*/, VariantMap &eventData) {
    auto *network = GetSubsystem<Network>();
...
String TrafficCity::SaveScene(bool initial) {
    String filename = "TrafficCity_sceneExport";
    if (!initial)
        filename += "InGame";
...
Controls TrafficCity::sample_input() {
    auto ui = GetSubsystem<UI>();
    auto input = GetSubsystem<Input>();
...
void TrafficCity::apply_input(Node *actorNode, const Controls &controls) {


}

void TrafficCity::apply_input(Connection *connection, const Controls &controls) {
/*
...
void TrafficCity::HandleClientIdentity(StringHash eventType, VariantMap& eventData) {
    // SERVER CODE
...
void TrafficCity::SendWelcomeMessage(Connection* connection) {
        Server *server = GetSubsystem<Server>();
        ...
        String text = "Welcome " + username + " to TrafficCity world!";
        // Send censored chat
        newEventData[P_MESSAGE] = text;
...
void TrafficCity::HandleSceneUpdate(StringHash eventType, VariantMap &eventData) {
...
SharedPtr<Node> TrafficCity::SpawnPlayer() {
...
// CLIENT CODE
void TrafficCity::CreateClientUI() {
...
void TrafficCity::DestroyPlayer(Connection *connection) {
...
Node *TrafficCity::SpawnPlayer(Connection *connection) {
...
NetworkActor *TrafficCity::SpawnPlayer(unsigned int id) {
...
void TrafficCity::InitiateGameMap(Scene *scene) {
...
/*
 ...
 //       object->SetMaterial(cache->GetResource<Material>("Materials/LOWPOLY_COLORS.xml")
    ...
    TrafficCity::InitEvolutionManager();
...
void TrafficCity::HandleClientDisconnected(StringHash eventType, VariantMap &eventData) {
    using namespace ClientConnected;
...
void TrafficCity::HandleServerConnected(StringHash eventType, VariantMap& eventData)
{
    StartMultiplayerGameSession();
    // Start in game mode
    UpdateUIState(true);
...
void TrafficCity::HandleServerDisconnected(StringHash eventType, VariantMap& eventData)
{

}

void TrafficCity::HandlePlayerRespawned(StringHash eventType, VariantMap& eventData)
{
...
void TrafficCity::HandlePlayerLoaded(StringHash eventType, VariantMap &eventData) {
...
void TrafficCity::HandleChatMessage(StringHash eventType, VariantMap &eventData) {
    using namespace ChatMessage;
    String textStr = "[" + eventData[P_USER].GetString() + "]: " + eventData[P_MESSAGE].GetString();
    ShowChatText(textStr);
}

void TrafficCity::PlayNextTrack() {
    // Go to next track
    radioTrack_++;
