...
#ifndef MM_AUDIOSPECTRUMANALYZER_H
#define MM_AUDIOSPECTRUMANALYZER_H
...
// FFT for audio spectrum analyzer
#include <Urho3D/ThirdParty/muFFT/fft.h>
#include <Urho3D/ThirdParty/muFFT/fft_internal.h>
...
public:
    ...
    AudioSpectrumAnalyzer(size_t inputSampleSize, size_t fftSampleSize, int samplingFrequency) :
            inputSize(0), fftSize(0), sampleFreq(44100), zeroLevel(0)
    {
    }
    ...
    void update(const Vector<float>* buffer, size_t headIndex, float minLevel, float maxLevel, float freqMin, float freqMax, float logBase)
    {
        ...
        {
            input[i] = buffer->At((headIndex + i) % buffer->Size());
        }
...
private:
    ...
    // AB: Patched for any index larger than 4096 is turning to +Inf
    float getPower(size_t index)
    {
        const float normalizeCoef = 2.0f / fftSize;
        cfloat a = output[index];

        // AB: Fix for larger than half of fft size -> set to zero
       if (index > (fftSize/2.0f)) {
            a.real = 0.0f;
            a.imag = 0.0f;
        }
...
#endif // MM_AUDIOSPECTRUMANALYZER_H