...
btScalar btRaycastVehicle::rayCast(btWheelInfo& wheel)
{
    ...
    // [AB] Wheel contact code

    // Cast a ray for each wheel, determine suspension based on hit distance and wheel radius
    // -> apply max suspension
    // -> wheel contact point -> hit point
    // -> m_suspensionRelativeVelocity -> suspension relative velocity
    //  \-> calculated based on projection of chassis velocity against contact normal
    //
...
void btRaycastVehicle::updateVehicle(btScalar step)
{
    ...
    // This is sigtraping for rayCast -> castRay -> rayTest -> btSingleRaycallback -> normalize -> fuzzy zero
    /* AB:
     * Assert !fuzzyZero() in line 307, file /code/dev/MonkeyMaya_com/Source/ThirdParty/Bullet/src/LinearMath/btVector3.h
...
void btRaycastVehicle::updateSuspension(btScalar deltaTime)
{
	...
	for (int w_it = 0; w_it < getNumWheels(); w_it++)
	{
		...
		else
		{
            ...
            //TODO: Enable downward sticky force -> when off track
...

	{
		for (int i = 0; i < getNumWheels(); i++)
		{
			...
			if (groundObject)
			{
                ...
                // From suggestion -> Lumak: changed from global to member variable
...
void* btDefaultVehicleRaycaster::castRay(const btVector3& from, const btVector3& to, btVehicleRaycasterResult& result)
{
    ...
    // AB: Add fix for fuzzy zero
    if ((from.length() > 19999.0f) || (to.length() > 19999.0f)) {
        return 0;
    }
	...
	if (rayCallback.hasHit())
	{
		...
		if (body && body->hasContactResponse())
		{
            ...
            // To avoid fuzzy zero, check hit normal length
            if (abs(result.m_hitNormalInWorld.length()) > 0.01f)
