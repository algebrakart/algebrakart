...
class URHO3D_API RaycastVehicleBase : public LogicComponent
{
    URHO3D_OBJECT(RaycastVehicleBase, LogicComponent);

public:
    /// Construct.
    explicit RaycastVehicleBase(Urho3D::Context* context);
    /// Destruct.
    ~RaycastVehicleBase() override;
    ...
    void SetWheelSideFrictionStiffness(int wheel, float stiffness);
    ...
    void DebugDraw(DebugRenderer *dbgRenderer);

    SharedPtr<RigidBody> GetBody() { return SharedPtr<RigidBody>(hullBody_); };

    int getNumWheelsContact() const;
    ...
    btWheelInfo& GetWheel(int wheel) const;
    ...
    float GetSpeedKm() const;
    float GetRPM() const;
    float GetDistanceOnGround() const;
...
private:
    ...
    bool m_onGround;
    float m_distanceOnGround_;
    float m_noWheelContactTime;

private:
    int m_numWheelsContact;
    int m_numWheelsPrevContact;

    Vector<int> m_wheelContact;
    Vector<int> m_wheelPrevContact;
public:
    const Vector<int> &getWheelContact() const;
    const Vector<int> &getWheelPrevContact() const;
    int getNumWheelsPrevContact() const;

    float getNoWheelContactTime() const;
