...
#include "RaycastVehicleBase.h"
#include "../../ThirdParty/Bullet/src/BulletDynamics/Vehicle/btRaycastVehicle.h"
...
const IntVector3 RaycastVehicleBase::RIGHT_UP_FORWARD(0, 1, 2);
const IntVector3 RaycastVehicleBase::RIGHT_FORWARD_UP(0, 2, 1);
const IntVector3 RaycastVehicleBase::UP_FORWARD_RIGHT(1, 2, 0);
const IntVector3 RaycastVehicleBase::UP_RIGHT_FORWARD(1, 0, 2);
const IntVector3 RaycastVehicleBase::FORWARD_RIGHT_UP(2, 0, 1);
const IntVector3 RaycastVehicleBase::FORWARD_UP_RIGHT(2, 1, 0);
...
RaycastVehicleBase::RaycastVehicleBase(Context* context) :
    LogicComponent(context)
{
    ...
    m_flip = false;
    m_flipTime = 0;
    m_distanceOnGround_ = 0.0f;
    m_onGround = false;

}

RaycastVehicleBase::~RaycastVehicleBase()
{
    delete vehicleData_;
    wheelNodes_.Clear();
}
...
void RaycastVehicleBase::RegisterObject(Context* context)
{
    context->RegisterFactory<RaycastVehicleBase>();
...
void RaycastVehicleBase::OnSetEnabled()
{
    if (vehicleData_)
        vehicleData_->SetEnabled(IsEnabledEffective());
}

void RaycastVehicleBase::ApplyAttributes()
{
    int index = 0;
...
void RaycastVehicleBase::FixedUpdate(float timeStep)
{
    ...
    // Store last non-zero contact
    if (m_numWheelsContact > 0) {
        m_numWheelsPrevContact = m_numWheelsContact;
        m_wheelPrevContact = m_wheelContact;
    }

    // Clear for new update
    m_onGround = false;
    m_numWheelsContact = 0;
    m_wheelContact.Clear();
    for (int i = 0; i < GetNumWheels(); i++) {
        if (WheelIsGrounded(i)) {
            m_numWheelsContact++;
            m_onGround = true;
            m_wheelContact.Push(i);
            m_noWheelContactTime = 0;
        }
    }

    if (!m_onGround) {
        m_noWheelContactTime += timeStep;
    }

    for (int i = 0; i < GetNumWheels(); i++) {
        btWheelInfo whInfo = vehicle->getWheelInfo(i);

        // Wheels on ground contribute to distance
        if (WheelIsGrounded(i)) {
            m_distanceOnGround_ += GetSpeedKm() * timeStep;
           // URHO3D_LOGDEBUGF("**RaycastVehicleBase::FixedUpdate %d -> [%d]->%d", id_, i, WheelIsGrounded(i));
            break;
        }
    }

    if (!m_onGround) {
        // If the car is moving fast, calculate distance even if not registering on-ground on wheels
/*        if (GetSpeedKm() > 90.0f) {
            m_distanceOnGround_ += GetSpeedKm() * timeStep;
        }*/
    }
...
void RaycastVehicleBase::FixedPostUpdate(float timeStep)
{
    btRaycastVehicle* vehicle = vehicleData_->Get();
...
void RaycastVehicleBase::SetMaxSideSlipSpeed(float speed)
{
    maxSideSlipSpeed_ = speed;
}

float RaycastVehicleBase::GetMaxSideSlipSpeed() const
{
    return maxSideSlipSpeed_;
}

void RaycastVehicleBase::SetWheelSkidInfoCumulative(int wheel, float skid)
{
    skidInfoCumulative_[wheel] = skid;
}

float RaycastVehicleBase::GetWheelSkidInfoCumulative(int wheel) const
{
    return skidInfoCumulative_[wheel];
}

void RaycastVehicleBase::AddWheel(Node* wheelNode,
...
void RaycastVehicleBase::ResetSuspension()
{
    btRaycastVehicle* vehicle = vehicleData_->Get();
    vehicle->resetSuspension();
}

void RaycastVehicleBase::UpdateWheelTransform(int wheel, bool interpolated)
{
    btRaycastVehicle* vehicle = vehicleData_->Get();
    vehicle->updateWheelTransform(wheel, interpolated);
}

Vector3 RaycastVehicleBase::GetWheelPosition(int wheel)
{
    btRaycastVehicle* vehicle = vehicleData_->Get();
...
Quaternion RaycastVehicleBase::GetWheelRotation(int wheel)
{
    btRaycastVehicle* vehicle = vehicleData_->Get();
...
Vector3 RaycastVehicleBase::GetWheelConnectionPoint(int wheel) const
{
    btRaycastVehicle* vehicle = vehicleData_->Get();
...
void RaycastVehicleBase::SetSteeringValue(int wheel, float steeringValue)
{
    btRaycastVehicle* vehicle = vehicleData_->Get();
    vehicle->setSteeringValue(steeringValue, wheel);
}

float RaycastVehicleBase::GetSteeringValue(int wheel) const
{
    btRaycastVehicle* vehicle = vehicleData_->Get();
    btWheelInfo whInfo = vehicle->getWheelInfo(wheel);
...
void RaycastVehicleBase::SetWheelSuspensionStiffness(int wheel, float stiffness)
{
    btRaycastVehicle* vehicle = vehicleData_->Get();
    btWheelInfo& whInfo = vehicle->getWheelInfo(wheel);
...
void RaycastVehicleBase::SetWheelSideFrictionStiffness(int wheel, float stiffness)
{
    btRaycastVehicle* vehicle = vehicleData_->Get();
    btWheelInfo& whInfo = vehicle->getWheelInfo(wheel);
...
float RaycastVehicleBase::GetWheelSuspensionStiffness(int wheel) const
{
    btRaycastVehicle* vehicle = vehicleData_->Get();
    btWheelInfo whInfo = vehicle->getWheelInfo(wheel);
...
void RaycastVehicleBase::SetWheelMaxSuspensionForce(int wheel, float force)
{
    btRaycastVehicle* vehicle = vehicleData_->Get();
    btWheelInfo& whInfo = vehicle->getWheelInfo(wheel);
...
float RaycastVehicleBase::GetWheelMaxSuspensionForce(int wheel) const
{
    btRaycastVehicle* vehicle = vehicleData_->Get();
    btWheelInfo whInfo = vehicle->getWheelInfo(wheel);
...
void RaycastVehicleBase::SetWheelDampingRelaxation(int wheel, float damping)
{
    btRaycastVehicle* vehicle = vehicleData_->Get();
    btWheelInfo& whInfo = vehicle->getWheelInfo(wheel);
...
float RaycastVehicleBase::GetWheelDampingRelaxation(int wheel) const
{
    btRaycastVehicle* vehicle = vehicleData_->Get();
    btWheelInfo whInfo = vehicle->getWheelInfo(wheel);
...
void RaycastVehicleBase::SetWheelDampingCompression(int wheel, float compression)
{
    btRaycastVehicle* vehicle = vehicleData_->Get();
    btWheelInfo& whInfo = vehicle->getWheelInfo(wheel);
...
float RaycastVehicleBase::GetWheelDampingCompression(int wheel) const
{
    btRaycastVehicle* vehicle = vehicleData_->Get();
    btWheelInfo whInfo = vehicle->getWheelInfo(wheel);
...
void RaycastVehicleBase::SetWheelFrictionSlip(int wheel, float slip)
{
    btRaycastVehicle* vehicle = vehicleData_->Get();
    btWheelInfo& whInfo = vehicle->getWheelInfo(wheel);
...
float RaycastVehicleBase::GetWheelFrictionSlip(int wheel) const
{
    btRaycastVehicle* vehicle = vehicleData_->Get();
    btWheelInfo whInfo = vehicle->getWheelInfo(wheel);
...
void RaycastVehicleBase::SetWheelRollInfluence(int wheel, float rollInfluence)
{
    btRaycastVehicle* vehicle = vehicleData_->Get();
    btWheelInfo& whInfo = vehicle->getWheelInfo(wheel);
...
Vector3 RaycastVehicleBase::GetContactPosition(int wheel) const
{
    btRaycastVehicle* vehicle = vehicleData_->Get();
    btWheelInfo& whInfo = vehicle->getWheelInfo(wheel);
...
Vector3 RaycastVehicleBase::GetContactNormal(int wheel) const
{
    btRaycastVehicle* vehicle = vehicleData_->Get();
    btWheelInfo& whInfo = vehicle->getWheelInfo(wheel);
...
float RaycastVehicleBase::GetWheelSideSlipSpeed(int wheel) const
{
    return wheelSideSlipSpeed_[wheel];
}

float RaycastVehicleBase::GetWheelRollInfluence(int wheel) const
{
    btRaycastVehicle* vehicle = vehicleData_->Get();
    btWheelInfo whInfo = vehicle->getWheelInfo(wheel);
...
void RaycastVehicleBase::SetWheelRadius(int wheel, float wheelRadius)
{
    btRaycastVehicle* vehicle = vehicleData_->Get();
    btWheelInfo& whInfo = vehicle->getWheelInfo(wheel);
...
float RaycastVehicleBase::GetWheelRadius(int wheel) const
{
    btRaycastVehicle* vehicle = vehicleData_->Get();
    btWheelInfo& whInfo = vehicle->getWheelInfo(wheel);
...
void RaycastVehicleBase::SetEngineForce(int wheel, float force)
{
    btRaycastVehicle* vehicle = vehicleData_->Get();
    vehicle->applyEngineForce(force, wheel);
}

float RaycastVehicleBase::GetEngineForce(int wheel) const
{
    btRaycastVehicle* vehicle = vehicleData_->Get();
    btWheelInfo whInfo = vehicle->getWheelInfo(wheel);
...
void RaycastVehicleBase::SetBrake(int wheel, float force)
{
    btRaycastVehicle* vehicle = vehicleData_->Get();
    vehicle->setBrake(force, wheel);
}

float RaycastVehicleBase::GetBrake(int wheel) const
{
    btRaycastVehicle* vehicle = vehicleData_->Get();
    btWheelInfo whInfo = vehicle->getWheelInfo(wheel);
...
int RaycastVehicleBase::GetNumWheels() const
{
    btRaycastVehicle* vehicle = vehicleData_->Get();
    return vehicle->getNumWheels();
}

Node* RaycastVehicleBase::GetWheelNode(int wheel) const
{
    return wheelNodes_[wheel];
}

void RaycastVehicleBase::SetMaxSuspensionTravel(int wheel, float maxSuspensionTravel)
{
    btRaycastVehicle* vehicle = vehicleData_->Get();
    btWheelInfo& whInfo = vehicle->getWheelInfo(wheel);
...
float RaycastVehicleBase::GetMaxSuspensionTravel(int wheel)
{
    btRaycastVehicle* vehicle = vehicleData_->Get();
    btWheelInfo whInfo = vehicle->getWheelInfo(wheel);
...
void RaycastVehicleBase::SetWheelDirection(int wheel, Vector3 direction)
{
...
Vector3 RaycastVehicleBase::GetWheelDirection(int wheel) const
{
    btRaycastVehicle* vehicle = vehicleData_->Get();
    btWheelInfo& whInfo = vehicle->getWheelInfo(wheel);
...
void RaycastVehicleBase::SetWheelAxle(int wheel, Vector3 axle)
{
...
Vector3 RaycastVehicleBase::GetWheelAxle(int wheel) const
{
    btRaycastVehicle* vehicle = vehicleData_->Get();
    btWheelInfo& whInfo = vehicle->getWheelInfo(wheel);
...
void RaycastVehicleBase::SetWheelRestLength(int wheel, float length)
{
    btRaycastVehicle* vehicle = vehicleData_->Get();
    btWheelInfo& whInfo = vehicle->getWheelInfo(wheel);
...
float RaycastVehicleBase::GetWheelRestLength(int wheel) const
{
    btRaycastVehicle* vehicle = vehicleData_->Get();
    btWheelInfo& whInfo = vehicle->getWheelInfo(wheel);
...
void RaycastVehicleBase::SetWheelSkidInfo(int wheel, float factor)
{
    btRaycastVehicle* vehicle = vehicleData_->Get();
    btWheelInfo& whInfo = vehicle->getWheelInfo(wheel);
...
float RaycastVehicleBase::GetWheelSkidInfo(int wheel) const
{
    btRaycastVehicle* vehicle = vehicleData_->Get();
    btWheelInfo& whInfo = vehicle->getWheelInfo(wheel);
...
bool RaycastVehicleBase::IsFrontWheel(int wheel) const
{
    btRaycastVehicle* vehicle = vehicleData_->Get();
    btWheelInfo& whInfo = vehicle->getWheelInfo(wheel);
...
bool RaycastVehicleBase::WheelIsGrounded(int wheel) const
{
    btRaycastVehicle* vehicle = vehicleData_->Get();
    btWheelInfo whInfo = vehicle->getWheelInfo(wheel);
...
void RaycastVehicleBase::SetInAirRPM(float rpm)
{
    inAirRPM_ = rpm;
}

float RaycastVehicleBase::GetInAirRPM() const
{
    return inAirRPM_;
}

void RaycastVehicleBase::SetCoordinateSystem(const IntVector3& coordinateSystem)
{
    coordinateSystem_ = coordinateSystem;
    vehicleData_->SetCoordinateSystem(coordinateSystem_);
}

void RaycastVehicleBase::ResetWheels()
{
    ResetSuspension();
    for (int i = 0; i < GetNumWheels(); i++)
    {
...
VariantVector RaycastVehicleBase::GetWheelDataAttr() const
{
    VariantVector ret;
...
void RaycastVehicleBase::SetWheelDataAttr(const VariantVector& value)
{
    if (!vehicleData_)
    {
...
void RaycastVehicleBase::Flip() {
    m_flip = true;
}

void RaycastVehicleBase::ApplyImpulse(Vector3 impulse) {
    hullBody_->ApplyImpulse(impulse);
}

float RaycastVehicleBase::GetSpeedKm() const {
    // Calculate the km/hr
    return vehicleData_->Get()->getCurrentSpeedKmHour();
}

float RaycastVehicleBase::GetDistanceOnGround() const {
    return m_distanceOnGround_;
}

btWheelInfo& RaycastVehicleBase::GetWheel(int wheel) const {
    return vehicleData_->Get()->getWheelInfo(wheel);
}

float RaycastVehicleBase::GetRPM() const {
        return 0;
}

int RaycastVehicleBase::getNumWheelsContact() const {
    return m_numWheelsContact;
}

const Vector<int> &RaycastVehicleBase::getWheelContact() const {
    return m_wheelContact;
}

const Vector<int> &RaycastVehicleBase::getWheelPrevContact() const {
    return m_wheelPrevContact;
}

int RaycastVehicleBase::getNumWheelsPrevContact() const {
    return m_numWheelsPrevContact;
}

float RaycastVehicleBase::getNoWheelContactTime() const {
    return m_noWheelContactTime;
}

void RaycastVehicleBase::DebugDraw(DebugRenderer *dbgRenderer) {
    ...
    for (int v = 0; v < vehicle->getNumWheels(); v++)
    {
        ...
        if (vehicle->getWheelInfo(v).m_raycastInfo.m_isInContact)
        {
        ...
        btVector3 wheelPosWS = vehicle->getWheelInfo(v).m_worldTransform.getOrigin();
        btVector3 contactPosWS = vehicle->getWheelInfo(v).m_raycastInfo.m_contactPointWS;

        btVector3 axle = btVector3(
                vehicle->getWheelInfo(v).m_worldTransform.getBasis()[0][vehicle->getRightAxis()],
                vehicle->getWheelInfo(v).m_worldTransform.getBasis()[1][vehicle->getRightAxis()],
                vehicle->getWheelInfo(v).m_worldTransform.getBasis()[2][vehicle->getRightAxis()]);

        btVector3 wheelSpin = btVector3(
                vehicle->getWheelInfo(v).m_worldTransform.getBasis()[0][vehicle->getUpAxis()],
                vehicle->getWheelInfo(v).m_worldTransform.getBasis()[1][vehicle->getUpAxis()],
